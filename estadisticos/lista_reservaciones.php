...
<head>
	<meta charset="UTF-8" />
	<title><?php echo $mes.'-'.$_GET["anno"];?></title>
...
<body>
    ...
    <?php
        ...
        $ResRes=mysqli_query($conn, "SELECT * FROM reservacion WHERE Fecha LIKE '".$anno."-".$mes."-%' ORDER BY Id DESC");
        while($RResRes=mysqli_fetch_array($ResRes))
        {
            ...
            echo '<tr>
                ...
                <td>'.strtoupper($ResPac["Curp"]).'</td>
                <td>'.$edad.'</td>
                <td>'.$ResPac["Talla"].'</td>
                <td>'.$ResPac["Peso"].'</td>
                <td>';if($ResPac["TelefonoFijo"]!=''){echo $ResPac["TelefonoFijo"];}if($ResPac["TelefonoCelular"]!=''){echo '<br />'.$ResPac["TelefonoCelular"];}if($ResPac["TelefonoContacto"]){echo '<br />'.$ResPac["TelefonoContacto"];} echo '</td>
            ...
            while($RResAcos=mysqli_fetch_array($ResAcos))
            {
                ...
                echo '<tr>
                    ...
                    <td>'.strtoupper($ResAcom["Curp"]).'</td>
                    <td>'.$edadaco.'</td>
                    <td>'.$ResAcom["Talla"].'</td>
                    <td>'.$ResAcom["Peso"].'</td>
                    <td>';if($ResAcom["TelefonoFijo"]!=''){echo $ResAcom["TelefonoFijo"];}if($ResAcom["TelefonoCelular"]!=''){echo '<br />'.$ResAcom["TelefonoCelular"];}if($ResAcom["TelefonoContacto"]){echo '<br />'.$ResAcom["TelefonoContacto"];} echo '</td>
                    <td>'.utf8_encode($EstadoA["Estado"]).'</td>
                    <td>'.utf8_encode($MunicipioA["Municipio"]).'</td>
                    <td>'.$direcciona.'</td>
                    <td colspan="2" align="center">Acompañante</td>
        ...
        //bitacora
        mysqli_query($conn, "INSERT INTO bitacora (FechaHora, IdUser, Hizo, Datos) VALUES ('".time()."', '".$_SESSION["Id"]."', '102', '".json_encode($_GET)."')");
    ?>
    </tbody>
</table>
</body>
