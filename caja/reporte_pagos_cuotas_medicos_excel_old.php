<?php 
//Inicio la sesion 
session_start();
include('../conexion.php');
include ("excelgen.class.php");

//initiate a instance of "excelgen" class
$excel = new ExcelGen("ReporteCuotasMedicos");

//initiate $row,$col variables
$row=0;
$col=0;

$excel->WriteText($row,$col,"N. Recibo");$col++;
$excel->WriteText($row,$col,"Fecha");$col++;
$excel->WriteText($row,$col,"N. Paciente");$col++;
$excel->WriteText($row,$col,"Paciente");$col++;
$excel->WriteText($row,$col,"Monto");$col++;

$row++;
$col=0;

$ResRecibos=mysqli_query($conn, "SELECT * FROM pagogastosmedicos WHERE Fecha>='".$_GET["fechaini"]."' AND Fecha<='".$_GET["fechafin"]."' ORDER BY Id DESC");
while($RResRec=mysqli_fetch_array($ResRecibos))
{
    $ResPaciente=mysqli_fetch_array(mysqli_query($conn, "SELECT Id, Nombre, Apellidos FROM pacientes WHERE Id='".$RResRec["IdPaciente"]."'"));
        
        $excel->WriteText($row,$col,$RResRec["Id"]);$col++;
		$excel->WriteText($row,$col,$RResRec["Fecha"]);$col++;
		$excel->WriteText($row,$col,$ResPaciente["Id"]);$col++;
		$excel->WriteText($row,$col,$ResPaciente["Nombre"].' '.$ResPaciente["Apellidos"]);$col++;
		$excel->WriteNumber($row,$col,$RResRec["CantidadEntregada"]);$col++;

        $row++;
		$col=0;
}

//bitacora
mysqli_query($conn, "INSERT INTO bitacora (FechaHora, IdUser, Hizo, Datos) VALUES ('".time()."', '".$_SESSION["Id"]."', '127', '".json_encode($_GET)."')");

//stream Excel for user to download or show on browser
$excel->SendFile();
?>


